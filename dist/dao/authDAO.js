...
class Auth {
    ...
    async getVerify(req) {
        ...
        const userData = await getUserData(oAuthClient.credentials.access_token);
        ...
        if (saveUser.account.length > 0) {
            for (let acc of saveUser.account) {
                ...
                if (account) {
                    ...
                    const update = await accountModel.findOneAndUpdate({ _id: account._id }, // Find the account by _id
            ...
            const update = await userModel.findByIdAndUpdate(saveUser?._id, { $addToSet: { account: newAccount?._id } }, { new: true });
            console.log({ update });
        }
    ...
    async login(req) {
        ...
        const passwordsMatch = await bcrypt.compare(password, isEmailExist?.password);
        if (!passwordsMatch) {
