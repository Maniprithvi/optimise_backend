class FacebookDAO {
    FACEBOOK_APP_SECRET = "cc819a109f793efd7c0d909ca0397451";
    FACEBOOK_APP_ID = "1197114374946072";
    ...
    async handleOAuth(req) {
        ...
        const tokenResponse = await fetch(`https://graph.facebook.com/v20.0/oauth/access_token?client_id=${this.FACEBOOK_APP_ID}&client_secret=${this.FACEBOOK_APP_SECRET}&redirect_uri=${REDIRECT_URI}&code=${code}`);
        if (!tokenResponse.ok) {
        ...
        const profileResponse = await fetch(`https://graph.facebook.com/v20.0/me?fields=name,email&access_token=${access_token}`);
        ...
        const pageToken = await fetch(`https://graph.facebook.com/${profile.id}/accounts?access_token=${access_token}`);
...
//# sourceMappingURL=faceBookDAO.js.map