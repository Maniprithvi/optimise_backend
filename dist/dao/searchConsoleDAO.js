...
class searchConsole {
    async generateRedirectUrl(req) {
        try {
            if (!process.env.GOOGLE_CLIENT_ID4 ||
            ...
            const authoriseUrl = Client.generateAuthUrl({
                scope: "openid email profile https://www.googleapis.com/auth/webmasters.readonly https://www.googleapis.com/auth/webmasters ",
                access_type: "offline",
                prompt: "consent",
            });
            return authoriseUrl;
    ...
    async getVerifyfromConsole(req) {
        ...
        const userData = await getUserData(consoleClient.credentials.access_token);
        const saveUser = await userModel.findOneAndUpdate({ email: userData.email }, {
            name: userData.name,
...
//# sourceMappingURL=searchConsoleDAO.js.map