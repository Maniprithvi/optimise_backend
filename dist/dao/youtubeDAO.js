...
import { getRefreshToken, getUserData } from "../helper/token.js";
...
class YoutubeDAO {
    async generateRedirectUrl(req) {
        ...
        // }
        try {
            if (!process.env.GOOGLE_CLIENT_ID2 ||
            ...
            const authoriseUrl = youtubeClient.generateAuthUrl({
                scope: "openid email profile https://www.googleapis.com/auth/youtube.readonly https://www.googleapis.com/auth/yt-analytics.readonly https://www.googleapis.com/auth/youtube https://www.googleapis.com/auth/youtube.force-ssl ",
                access_type: "offline",
                prompt: "consent",
            });
    ...
    async getVerifyfromYoutube(req) {
        ...
        const userData = await getUserData(youtubeClient.credentials.access_token);
        ...
        if (saveUser.account.length > 0) {
            ...
            if (youtueAcc) {
                ...
                const update = await accountModel.findOneAndUpdate({ _id: youtueAcc?._id }, // Find the account by _id
    ...
    async getAnalyticsData(req) {
        ...
        if (Date.now() > Number(token)) {
            ...
            const refreshToken = await getRefreshToken(tokenObj);
        ...
        const data = await youtubeanalytics.reports.query({
            ...
            startDate: startDate,
            endDate: getDate(),
            metrics: "views,likes,subscribersGained,estimatedMinutesWatched,comments,dislikes,shares,videosAddedToPlaylists",
            dimensions: "day",
            sort: "day",
        });
        return data.data;
    ...
    async getvideosInfo(req) {
        ...
        do {
            try {
                ...
                const ids = uploads?.map((item) => item?.contentDetails?.upload?.videoId);
...
//# sourceMappingURL=youtubeDAO.js.map